...
From @fig-time , we could see that the obesity prevalence proportion first decreased from 2008 to 2009 and got the lowest value 28.78%. Then it increased dramatically and reached its peak value, 30.47% , in 2010. After 2010, it continued to decrease to 29.61% in 2012.
...
#| echo: false
#| label: fig-sex
#| fig-cap: Confidence Interval Plot for Sex
data <- data |>
...
| term      | estimate | p-value         |
|-----------|----------|-----------------|
| Intercept | -1.73631 | \<2e-16 \*\*\*  |
| 25-34     | 0.44453  | 2.68e-09 \*\*\* |
| 35-44     | 0.88317  | \<2e-16 \*\*\*  |
| 45-54     | 0.95485  | \<2e-16 \*\*\*  |
| 55-64     | 1.16308  | \<2e-16 \*\*\*  |
| 65-74     | 1.11899  | \<2e-16 \*\*\*  |
...
: Regression output of obesity by age group {#tbl-agereg}
...
The model equation from @tbl-agereg is
...
: Probability of Obesity by Age Group {#tbl-ageprob}

We can see from @tbl-ageprob the probability of being obese is highest for the 55-64 age group and lowest for the 16-24. We can plot the estimated probabilities by age group.

```{r}
#| echo: false
#| label: fig-ageci
#| fig-cap: Confidence Interval Plot for Age Group
...
We can see from @fig-ageci that there is quite a lot of difference between the estimated probabilities of being obese in each group.
...
| term                                                | estimate | p-value         |
|-------------------------------------------|---------------|---------------|
| Intercept                                           | -0.86824 | 5.45e-11 \*\*\* |
| In full time education                              | -1.16236 | 1.68e-12 \*\*\* |
| In paid employment, self employed or gov't training | -0.03470 | 0.79521         |
| Looking after home/family                           | -0.06153 | 0.66875         |
| Looking for/ intending to look for paid work        | -0.12037 | 0.43834         |
| Perm unable to work                                 | 0.41501  | 0.00444 \*\*    |
| Retired                                             | 0.16511  | 0.22132         |

: Regression Output for Obesity by Employment Status {#tbl-employ}
...
For socio-economic status, from @tbl-employ, we have the following model,

$$
\mbox{ln}({\frac{p}{1-p}}) = \alpha + -1.16 \cdot \mathbb{I}_{\mbox{Edu}}(x) + -0.03 \cdot \mathbb{I}_{\mbox{Emp}}(x) + -0.06 \cdot 
...
| Employment Status                                   | Probability |
|-----------------------------------------------------|-------------|
| Doing something else                                | 0.2956204   |
| In full time education                              | 0.1160275   |
| In paid employment, self employed or gov't training | 0.2884464   |
| Looking after home/family                           | 0.2829706   |
| Looking for/ intending to look for paid work        | 0.2711864   |
| Perm unable to work                                 | 0.3885918   |
| Retired                                             | 0.3311176   |

: Probability of Obesity Given Employment Status {#tbl-employprob}

@tbl-employprob shows that the probability of being obese is highest for subjects that are permanently unable to work, it is lowest for subjects in full-time education.

```{r}
#| echo: false
#| label: fig-socioeco
#| fig-cap: Confidence Interval Plot for Socio-Economic Status
...
From @fig-socioeco we can see there appears to be a significant difference in the probability of being obese between different socio-economic groups.

# Conclusions {#sec-C}

In our exploratory analysis in @sec-EA we investigated trends in obesity over the time period of 2008-2012. We saw that obesity initially decreased but then sharply increased in 2010 before decreasing again over the following years.

From the formal analysis in @sec-FA we see in our model considering the sex variable that sex is not a statistically significant predictor of obesity prevalence. Although the estimated probability of obesity appears to be slightly higher for females which is also what was found in exploratory analysis.
...
While our analysis provides insight into the effect of age and socio-economic status on obesity, several limitations are observed. The sample size may be considered unbalanced as the population sizes of each level in the categorical variable are not similar.

Furthermore, there maybe confounding variables which were not accounted for, such as additional socioeconomic factors, which may impact the results. Our analysis highlights the importance of age and socio-economic status as they have clear impact on the obesity prevalence , therefore more attention should be paid to 55-64 age group and "unable to work" group. Future studies should consider following the experimental population over a longer time period to see if there will be changes in the status of obesity.Finally, they should also consider utilizing a more balanced sample and control for additional confounding variables.
